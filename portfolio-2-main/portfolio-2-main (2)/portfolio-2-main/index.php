<!DOCTYPE html>

<html lang="en">
    <head>
        <meta charset="UTF-8">
        <meta http-equiv="X-UA-Compatible" content="IE-edge">
        <meta name="viewport" content="device-width, initial-scale=1.0">
        <title>BIBI Books</title>
        <link rel="stylesheet" href="styles.css">
    </head>
    <body>
      <div class="header">
            <a href="index.php" class="logo"><span class="leftName">BIBI</span> <span class="rightName">Books</span></a>
            <div class="headerRight">
              <a href="index.php">Home</a>
              <a href="shop.php">Shop</a>
              <a href="cart.php">Cart</a>
            </div>
        </div>

        <div class="welcomeTitle">
          <div class="left">
            <div class="welcomeImage">
              <img src="images/personReading.jpg" alt="person reading a book">
            </div>
          </div>
          <div class="right">
            <div class="welcomeMessage">
              <p>
                Welcome to BIBI Books, the ultimate hub for book lovers who 
                appreciate content and responsible sourcing. We celebrate the
                magic of literature and its ability to inspire and connect people.
                Our goal is to provide a special reading experience with a diverse
                range of ethically sourced books.
              </p>
            </div>
          </div>
        </div>

        <div class="welcomeTitle">
          <div class="whatWeOffer">
            <div><a class="whatWeOfferTitle">Amazing titles that we offer!</a></div>
            <div class="bookShowcase">
              <div class="bookOne">
                <img id="bookImage" src="images/pride-and-prejudice.jpg" alt="Front cover of the pride and prejudice book">
                <div class="bookInfo">
                  <p class="bookTitleHome">Pride and Prejudice</p>
                  <p class="author">by Jane Austen</p>
                  <p class="price">£24.99 GBP</p>
                </div>
              </div>
              <div class="bookTwo">
                <img id="bookImage" src="images/the-color-purple.jpeg" alt="Front cover of the color purple book">
                <div class="bookInfo">
                  <p class="bookTitleHome">The Color Purple</p>
                  <p class="author">by Alice Walker</p>
                  <p class="price">£22.99 GBP</p>
                </div>
              </div>
              <div class="bookThree">
                <img id="bookImage" src="images/where-the-crawdads-sing.jpg" alt="Front cover of where the crawdads sing book">
                <div class="bookInfo">
                  <p class="bookTitleHome">Where The Crawdads Sing</p>
                  <p class="author">by Delia Owens</p>
                  <p class="price">£27.99 GBP</p>
                </div>
              </div>
              
            </div>
            <div><a href="shop.php" class="whatWeOfferLink">Click here to see more!</a></div>
          </div>
        </div>

        <div class="welcomeTitle">
          <div class="left">
            <div class="welcomeImage">
              <img src="images/responsible-sourcing.jpeg">
            </div>
          </div>
          <div class="right">
            <div class="welcomeMessage">
              <p>
                We are proud to offer a wide selection of titles that are not only
                captivating but also environmentally friendly. Our commitment to 
                sustainability means that all our books are sourced from certified,
                eco-conscious suppliers who prioritize the responsible use of resources. 
              </p>
            </div>
          </div>
        </div>

        <div class="returnToTop">
          <div>
            <a class="returnButton" href="#">Return to top</a>
          </div>
        </div>
    </body>
</html>